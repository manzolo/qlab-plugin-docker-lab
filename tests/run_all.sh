#!/usr/bin/env bash
# run_all.sh — Run all docker-lab exercise tests
...
should_skip() {
    local num="$1"
    for s in "${SKIP_TESTS[@]+"${SKIP_TESTS[@]}"}"; do [[ "$num" == "$s" ]] && return 0; done
    return 1
}

echo ""
echo "${BOLD}=========================================${RESET}"
echo "${BOLD}  docker-lab — Automated Test Suite${RESET}"
echo "${BOLD}=========================================${RESET}"
echo ""
...
log_info "Checking Docker is ready..."
assert "Docker daemon is running" ssh_vm "docker info >/dev/null 2>&1"
...
run_test() {
    local num="$1"
    local files=($TESTS_DIR/test_${num}_*.sh)
    ...
    if [[ "$test_exit" -ne 0 ]]; then TOTAL_FAIL=$((TOTAL_FAIL + 1)); FAILED_EXERCISES+=("$num")
...
if [[ "$TOTAL_FAIL" -gt 0 ]]; then
    echo ""; printf "${RED}${BOLD}  FAILED exercises: %s${RESET}\n" "${FAILED_EXERCISES[*]}"; exit 1
else
    echo ""; printf "${GREEN}${BOLD}  All exercises passed!${RESET}\n"; exit 0
fi
