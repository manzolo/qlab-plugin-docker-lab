#!/usr/bin/env bash
# Common helpers for docker-lab test suite
...
SERVER_PORT="$(_get_port docker-lab)"
if [[ -z "$SERVER_PORT" ]]; then
    echo "ERROR: Cannot find VM port. Is docker-lab running?"; exit 1
fi
...
cleanup_docker() {
    ...
    ssh_vm "docker rm -f web mybox logger vol-test myapp 2>/dev/null; docker volume rm mydata 2>/dev/null; docker rmi myapp:v1 2>/dev/null; rm -rf ~/myapp /tmp/testfile.txt /tmp/hostdir 2>/dev/null" 2>/dev/null || true
}
